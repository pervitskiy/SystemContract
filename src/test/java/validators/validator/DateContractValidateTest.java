...
public class DateContractValidateTest extends TestCase {
    ...
    Contract contract3 = new InternetContract(3,3, LocalDate.of(2009, 10, 12),LocalDate.of(2008, 12, 01), person2, 110);
    Contract contract4 = new TVContract(4,4, LocalDate.of(2009, 10, 12),LocalDate.of(2009, 10, 11), person2, TVContract.ChannelPackage.CINEMA);
    Contract contract5 = new MobileContract(6,5, LocalDate.of(2009, 10, 12),LocalDate.of(2012, 12, 01), person3, new Rate(120, 100, 8 ));

    DateContractValidate dateContractValidate = new DateContractValidate();

    public void testDoValidate() {
        Assert.assertEquals(dateContractValidate.doValidate(contract1).getStatus(), ValidationStatus.ОК);
        Assert.assertEquals(dateContractValidate.doValidate(contract2).getStatus(), ValidationStatus.ОК);
        Assert.assertEquals(dateContractValidate.doValidate(contract3).getStatus(), ValidationStatus.ERROR);
        Assert.assertEquals(dateContractValidate.doValidate(contract4).getStatus(), ValidationStatus.ERROR);
        Assert.assertEquals(dateContractValidate.doValidate(contract5).getStatus(), ValidationStatus.ОК);
